...
export default {
    ...
    methods: {
        ...
        getBackgroundImage() {
            return this.infoMovie.poster_path 
                ? `url(https://image.tmdb.org/t/p/w342${this.infoMovie.poster_path})`
                : `url(${fallbackPoster})`;
        },
        getCast(id){
            axios.get(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=6d80c7fc6fb3c8cde8ea957eacd0ae7c&language=it-IT`)
            .then(res => {
                // console.log(res.data.cast.slice(0, 5));
                this.movieCast = res.data.cast.slice(0, 5);
            })
            .catch(err => {
                console.log(err);
            })
        ...
        <div class="info-box">
            <ul class="info">
                ...
                <li>
                    <strong>Cast: </strong>
                    <ul class="cast">
                        <li v-for="(actor, i) in movieCast" :key="i">
                            {{ actor.name }}, 
                        </li>
                    </ul>
